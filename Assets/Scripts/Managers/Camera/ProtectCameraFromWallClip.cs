...
namespace UnityStandardAssets.Cameras
{
	public class ProtectCameraFromWallClip : MonoBehaviour
	{
		...
		[SerializeField] private Transform _camera;                  // the transform of the camera
		[SerializeField] private Transform _Pivot;                // the point at which the camera pivots around
		...
		private void Start()
		{
			...
			m_OriginalDist = _camera.localPosition.magnitude;
		...
		private void LateUpdate()
		{
			...
			m_Ray.origin = _Pivot.position + _Pivot.forward * sphereCastRadius;
			m_Ray.direction = -_Pivot.forward;
			...
			if (initialIntersect)
			{
				m_Ray.origin += _Pivot.forward * sphereCastRadius;
			...
			for (int i = 0; i < m_Hits.Length; i++)
			{
					  ...
					  m_Hits[i].collider.attachedRigidbody.CompareTag(dontClipTag)))
				{
					...
					targetDist = -_Pivot.InverseTransformPoint(m_Hits[i].point).z;
					hitSomething = true;
				}
			}
			...
			if (hitSomething)
			{
				Debug.DrawRay(m_Ray.origin, -_Pivot.forward * (targetDist + sphereCastRadius), Color.red);
			}
			...
			_camera.localPosition = -Vector3.forward * m_CurrentDist;
		}
